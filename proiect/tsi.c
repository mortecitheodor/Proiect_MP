...
void Detect_Direction(uint16_t current_x, uint16_t previous_x, uint16_t current_y, uint16_t previous_y, int8_t *dir_x, int8_t *dir_y) {
    *dir_x = (current_x > previous_x + 100) ? 1 : (current_x < previous_x - 100) ? -1 : 0;
    *dir_y = (current_y > previous_y + 100) ? 1 : (current_y < previous_y - 100) ? -1 : 0;
}
